...
fn test_iterative_version() {
    const DEFAULT_TOL: f64 = 1e-30;
    assert_eq!(lambert_w(0, NEG_INV_E, 0.0, DEFAULT_TOL), (-1.0, 0.0));
    assert_eq!(lambert_w(0, 1.0, 0.0, DEFAULT_TOL), (OMEGA, 0.0));
    assert_eq!(
        lambert_w(0, core::f64::consts::E, 0.0, DEFAULT_TOL),
        (1.0, 0.0)
    );
    assert_eq!(
        lambert_w(0, 2.0, 0.0, DEFAULT_TOL),
        (0.8526055020137255, 0.0)
    );
    assert_eq!(lambert_w(0, 0.0, 0.0, DEFAULT_TOL), (0.0, 0.0));
    assert_eq!(
        lambert_w(1, 0.0, 0.0, DEFAULT_TOL),
        (f64::NEG_INFINITY, 0.0)
    );

    assert_complex_approx_eq!(
        lambert_w(0, NEG_INV_E + 0.1, 0.0, DEFAULT_TOL),
        (-0.399_382_452_539_780_7, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_w(1, NEG_INV_E + 0.1, -1.0, DEFAULT_TOL),
    ...
    assert_complex_approx_eq!(
        lambert_w(-1, 0.5, 0.0, DEFAULT_TOL),
        (
    ...
    assert_complex_approx_eq!(
        lambert_w(0, 10.0, 0.0, DEFAULT_TOL),
        (1.745528002740699, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_w(0, 100.0, 0.0, DEFAULT_TOL),
        (3.385_630_140_290_05, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_w(0, 1000.0, 0.0, DEFAULT_TOL),
        (5.249_602_852_401_596, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_w(0, 10000.0, 0.0, DEFAULT_TOL),
        (7.231_846_038_093_373, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_w(-1, -f64::ln(2.0) / 2.0, 0.0, DEFAULT_TOL),
        (-f64::ln(4.0), 0.0)
    );
    // Close to the branch cut
    assert_complex_approx_eq!(lambert_w(-1, NEG_INV_E, 0.0, DEFAULT_TOL), (-1.0, 0.0));
    assert_complex_approx_eq!(
        lambert_w(10, NEG_INV_E + 0.1, 0.0, DEFAULT_TOL),
        (-5.484_673_997_441_509, 64.317_580_321_338_81)
    );
    assert_complex_approx_eq!(
        lambert_w(1, -1e-05, -1e-05, DEFAULT_TOL),
        (-1.37923465336253e+01, 8.46711143530535e-01),
        max_relative = 8.0 * f64::EPSILON
    );
    // Very big branch index
    assert_complex_approx_eq!(
        lambert_w(1_000_000, 100.0, 100.0, DEFAULT_TOL),
        (-10.701_643_723_106_727, 6.283_184_521_779_72e6)
    );
    // NaNs
    assert!(lambert_w(0, f64::NAN, 0.0, DEFAULT_TOL).0.is_nan());
    assert!(lambert_w(0, f64::NAN, 0.0, DEFAULT_TOL).1.is_nan());
    assert!(lambert_w(0, 0.0, f64::NAN, DEFAULT_TOL).0.is_nan());
    assert!(lambert_w(0, 0.0, f64::NAN, DEFAULT_TOL).1.is_nan());
    assert!(lambert_w(0, f64::NAN, f64::NAN, DEFAULT_TOL).0.is_nan());
    assert!(lambert_w(0, f64::NAN, f64::NAN, DEFAULT_TOL).1.is_nan());
    // Infinity
    assert!(lambert_w(0, f64::INFINITY, 0.0, DEFAULT_TOL).0.is_nan());
    assert!(lambert_w(0, f64::INFINITY, 0.0, DEFAULT_TOL).1.is_nan());
    assert!(lambert_w(0, 0.0, f64::INFINITY, DEFAULT_TOL).0.is_nan());
    assert!(lambert_w(0, 0.0, f64::INFINITY, DEFAULT_TOL).1.is_nan());
    assert!(lambert_w(0, f64::INFINITY, f64::INFINITY, DEFAULT_TOL)
        .0
        .is_nan());
    assert!(lambert_w(0, f64::INFINITY, f64::INFINITY, DEFAULT_TOL)
        .1
...
fn test_32_bit_iterative_version() {
    const DEFAULT_TOL: f32 = 1e-15;
    assert_eq!(
        lambert_wf(0, NEG_INV_E as f32, 0.0, DEFAULT_TOL),
        (-1.0, 0.0)
    );
    assert_eq!(lambert_wf(0, 1.0, 0.0, DEFAULT_TOL), (OMEGA as f32, 0.0));
    assert_eq!(
        lambert_wf(0, core::f32::consts::E, 0.0, DEFAULT_TOL),
        (1.0, 0.0)
    );
    assert_eq!(lambert_wf(0, 2.0, 0.0, DEFAULT_TOL), (0.852_605_5, 0.0));
    assert_eq!(lambert_wf(0, 0.0, 0.0, DEFAULT_TOL), (0.0, 0.0));
    assert_eq!(
        lambert_wf(1, 0.0, 0.0, DEFAULT_TOL),
        (f32::NEG_INFINITY, 0.0)
    );

    assert_complex_approx_eq!(
        lambert_wf(0, NEG_INV_E as f32 + 0.1, 0.0, DEFAULT_TOL),
        (-0.399_382_44, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_wf(1, NEG_INV_E as f32 + 0.1, -1.0, DEFAULT_TOL),
    ...
    assert_complex_approx_eq!(
        lambert_wf(-1, 0.5, 0.0, DEFAULT_TOL),
        (-2.259_158_8, -4.220_961)
    );
    assert_complex_approx_eq!(lambert_wf(0, 10.0, 0.0, DEFAULT_TOL), (1.745_528, 0.0));
    assert_complex_approx_eq!(lambert_wf(0, 100.0, 0.0, DEFAULT_TOL), (3.385_630_1, 0.0));
    assert_complex_approx_eq!(lambert_wf(0, 1000.0, 0.0, DEFAULT_TOL), (5.249_603, 0.0));
    assert_complex_approx_eq!(lambert_wf(0, 10000.0, 0.0, DEFAULT_TOL), (7.231_846, 0.0));
    assert_complex_approx_eq!(
        lambert_wf(-1, -f32::ln(2.0) / 2.0, 0.0, DEFAULT_TOL),
        (-f32::ln(4.0), 0.0)
    );
    // Close to the branch cut
    assert_complex_approx_eq!(
        lambert_wf(-1, NEG_INV_E as f32, 0.0, DEFAULT_TOL),
        (-1.0, 0.0)
    );
    assert_complex_approx_eq!(
        lambert_wf(10, NEG_INV_E as f32 + 0.1, 0.0, DEFAULT_TOL),
        (-5.484_674, 64.317_58)
    );
    // Very big branch index
    assert_complex_approx_eq!(
        lambert_wf(32767, 100.0, 100.0, DEFAULT_TOL),
        (-7.2833066, 205880.34)
    );
    // NaNs
    assert!(lambert_wf(0, f32::NAN, 0.0, DEFAULT_TOL).0.is_nan());
    assert!(lambert_wf(0, f32::NAN, 0.0, DEFAULT_TOL).1.is_nan());
    assert!(lambert_wf(0, 0.0, f32::NAN, DEFAULT_TOL).0.is_nan());
    assert!(lambert_wf(0, 0.0, f32::NAN, DEFAULT_TOL).1.is_nan());
    assert!(lambert_wf(0, f32::NAN, f32::NAN, DEFAULT_TOL).0.is_nan());
    assert!(lambert_wf(0, f32::NAN, f32::NAN, DEFAULT_TOL).1.is_nan());
    // Infinity
    assert!(lambert_wf(0, f32::INFINITY, 0.0, DEFAULT_TOL).0.is_nan());
    assert!(lambert_wf(0, f32::INFINITY, 0.0, DEFAULT_TOL).1.is_nan());
    assert!(lambert_wf(0, 0.0, f32::INFINITY, DEFAULT_TOL).0.is_nan());
    assert!(lambert_wf(0, 0.0, f32::INFINITY, DEFAULT_TOL).1.is_nan());
    assert!(lambert_wf(0, f32::INFINITY, f32::INFINITY, DEFAULT_TOL)
        .0
        .is_nan());
    assert!(lambert_wf(0, f32::INFINITY, f32::INFINITY, DEFAULT_TOL)
        .1
