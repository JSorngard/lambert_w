const TABLE: [(f64, [f64; 5], [f64; 5]); 19] = [
    (
        2.008_217_811_584_472_7,
        [
...
pub fn sw0(z: f64) -> f64 {
    ...
    let (_, p, q) = TABLE[..TABLE.len() - 1]
        .iter()
        .find(|(w, _, _)| z <= *w)
        .unwrap_or_else(|| TABLE.last().unwrap());

    let x = if z <= 4.103_565_939_888_539_6e20 {
        (z - NEG_INV_E).sqrt()
    } else {
        z.ln()
    };
